...
def show_point_to_point():
    ...
    st.html(
        ...
        <style>
            div[data-testid="stButton"] {
                ...
                /*
                Mobile에서도 버튼 폭이 너무 작아져
                Main 글자가 줄바꿈되지 않도록 최소 크기를 둡니다.
                PC에서는 기존 크기와 비슷하게 유지됩니다.
                */
                right: clamp(10px, 2.5vw, 25px) !important;
                top: clamp(10px, 2vw, 20px) !important;

                width: clamp(60px, 6.5vw, 72px) !important;
                height: clamp(28px, 2.7vw, 32px) !important;
    ...
    <head>
        ...
        <style>
            ...
            /*
            Mobile pinch zoom:
            브라우저 전체 페이지가 아니라 시뮬레이터 Stage 자체를 확대합니다.
            */
            #ptp-stage-wrap {{
                touch-action: none;
    ...
    <body>
        <div class="web-stage-wrap" id="ptp-stage-wrap">
        ...
        (() => {{
            ...
            // =================================================
            // Universal Stage pinch zoom / pan
            // - Two fingers: pinch zoom
            // - One finger while zoomed: pan
            // - Pinch back to 1.0: reset
            // PC mouse behavior is unchanged.
            // =================================================
            function installStagePinchZoom(
                wrap,
                stage
            ) {{
                if (!wrap || !stage) {{
                ...
                let panStartX = 0.0;
                let panStartY = 0.0;
                let panStartTranslateX = 0.0;
                let panStartTranslateY = 0.0;
                let panning = false;
                ...
                function midpointRelativeToWrap(
                    t0,
                    t1
                ) {{
                    const wrapRect =
                ...
                wrap.addEventListener(
                    ...
                    event => {{
                        if (event.touches.length === 2) {{
                            ...
                            const stageScreenLeft =
                                stageRect.left
                                - wrapRect.left;

                            const stageScreenTop =
                                stageRect.top
                                - wrapRect.top;
...
def show_central_placeholder():
    ...
    <body>
        <div class="web-stage-wrap" id="ccu-stage-wrap">
